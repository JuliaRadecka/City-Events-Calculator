...
/* Read-only blocks */
.ro-field { margin: 0 0 0.85rem 0; }
.ro-label { font-size: 0.85rem; opacity: 0.85; margin-bottom: 0.2rem; }
.ro-value {
...
/* Blue ONLY for disabled inputs (AUTO fields only) */
...
.ag-theme-alpine {
  ...
  --ag-foreground-color: rgba(255,255,255,0.88);
...
def aggrid_table(
    ...
    for col in df.columns:
        ...
        if 'auto_unique_id' in ncol:
            ...
            gb.configure_column(
                col,
                cellStyle={"backgroundColor": "#9DC3E6", "color": "#000"},
                editable=False,
            )
