...
def main():
    ...
    # Fail fast with a visible error instead of a 502 on Streamlit Cloud.
    # (A crash before any UI is rendered often shows as 502.)
    try:
        if not os.path.exists(WORKBOOK_PATH):
            st.error(
                "Не найден Excel-файл рядом с приложением. "
                "Проверьте имя файла в репозитории и путь в коде."
            )
        ...
        st.error("Ошибка при загрузке Excel-шаблона (на старте приложения).")
        st.exception(e)
        st.stop()
    ...
    # Build initial value map ONLY for the working areas.
    # IMPORTANT: Scanning the full sheet (ws.max_row/ws.max_column) can be extremely slow on Streamlit Cloud
    # if the workbook has formatting extended to large ranges. This is a typical reason for 502/timeouts.
